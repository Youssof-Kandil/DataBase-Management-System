...
public class Page implements Serializable {
	...
	public static Vector<Tuples> selectSingleSQLTermFromPage(SQLTerm sqlTerm,
			Iterator iterator , Iterator temp) throws DBAppException {
		Hashtable<String, Object> toBeSelected = new Hashtable();
		toBeSelected.put(sqlTerm._strColumnName, sqlTerm._objValue);
		String operator = sqlTerm._strOperator;
		Vector<Tuples> records = new Vector();

		switch (operator) {
		case "=":
			records = equal(toBeSelected, iterator);
			break;
		case "!=":
			records= greaterThan(toBeSelected, iterator);
			
			records.addAll(lessThan(toBeSelected, temp));
			break;
		case "<=":
			
			records= lessThan(toBeSelected, iterator);
			records.addAll(equal(toBeSelected, temp));
			break;
		case ">=":
			records= greaterThan(toBeSelected, iterator);
			records.addAll(equal(toBeSelected, temp));
		...
		case "<":
			...
			break;
		default:
			throw new DBAppException("You can't see me");
		}
		return records;

	}

	private static Vector<Tuples> lessThan(Hashtable<String, Object> toBeSelected, Iterator<Tuples> iterator) {
	    ...
	    while (iterator.hasNext()) {
	        ...
	        boolean found = true;
	        Object key = toBeSelected.keySet().iterator().next();
	        if (compareToo(toBeSelected.get(key), pageContent.get(key)) <= 0) {
	            found = false;
	        }
	...
	private static Vector<Tuples> greaterThan(
			Hashtable<String, Object> toBeSelected, Iterator<Tuples> iterator) {
			    Vector<Tuples> result = new Vector<Tuples>();
	...
	public static Vector<Tuples> equal(Hashtable<String, Object> toBeSelected, Iterator<Tuples> iterator) {
	    Vector<Tuples> result = new Vector<Tuples>();
	    ...
	    while (iterator.hasNext()) {
	        ...
	        for (String key : toBeSelected.keySet()) {
	            if (!toBeSelected.get(key).equals(pageContent.get(key))) {
	                found = false;
	                break;
	            }
	        }
	...
	public static String checkType(Object key) {
